...
export const FinanceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addAccount = async (account: Omit<Account, 'id' | 'balance' | 'createdAt' | 'updatedAt'>): Promise<boolean> => {
    ...
    try {
      ...
      const errorMessage = `Erro detalhado ao adicionar CONTA: ${error.message}`;
  ...
  const updateAccount = async (id: string, updatedFields: Partial<Account>): Promise<boolean> => {
    ...
    try {
      ...
      const errorMessage = `Erro detalhado ao atualizar CONTA: ${error.message}`;
      console.error(errorMessage);
      alert(errorMessage);
      return false;
    }
  ...
  const deleteAccount = async (id: string): Promise<boolean> => {
    ...
    try {
      ...
      const errorMessage = `Erro detalhado ao apagar CONTA: ${error.message}`;
      console.error(errorMessage);
      alert(errorMessage);
      return false;
    }
  ...
  const addCategory = async (category: Omit<Category, 'id' | 'isActive'>): Promise<boolean> => {
    ...
    try {
      ...
      const errorMessage = `Erro detalhado ao adicionar CATEGORIA: ${error.message}`;
      console.error(errorMessage);
      alert(errorMessage);
      return false;
    }
  ...
  const updateCategory = async (id: string, updatedFields: Partial<Category>): Promise<boolean> => {
    ...
    try {
      ...
      const errorMessage = `Erro detalhado ao atualizar CATEGORIA: ${error.message}`;
      console.error(errorMessage);
      alert(errorMessage);
      return false;
    }
  ...
  const deleteCategory = async (id: string): Promise<boolean> => {
    ...
    try {
      ...
      const errorMessage = `Erro detalhado ao apagar CATEGORIA: ${error.message}`;
      console.error(errorMessage);
      alert(errorMessage);
      return false;
    }
  ...
  const addTransaction = async (transaction: Omit<Transaction, 'id' | 'createdAt' | 'updatedAt'>): Promise<boolean> => {
    ...
    try {
      ...
      if (error) throw error;
      const newTransaction: Transaction = { id: dbRecord.id, description: dbRecord.descricao, amount: dbRecord.valor, date: dbRecord.data, type: transaction.type, accountId: dbRecord.conta_id, category: dbRecord.category, createdAt: dbRecord.created_at, updatedAt: dbRecord.created_at, recurrence: transaction.recurrence };
      ...
      const errorMessage = `Erro detalhado ao adicionar TRANSAÇÃO: ${error.message}`;
      console.error(errorMessage);
      alert(errorMessage);
      return false;
    }
  ...
  const deleteTransaction = async (id: string): Promise<boolean> => {
    ...
    try {
      ...
      const errorMessage = `Erro detalhado ao apagar TRANSAÇÃO: ${error.message}`;
      console.error(errorMessage);
      alert(errorMessage);
      return false;
    }
